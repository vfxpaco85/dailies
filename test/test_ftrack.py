...
from dailies.tracking import FtrackTracking


class TestFtrackTracking(unittest.TestCase):

    @patch("requests.post")
    def test_insert_version(self, mock_post):
        """
        Test the insert_version method for the Ftrack API.
        """
        mock_response = Mock()
        mock_response.status_code = 201  # Simulate a successful response
        mock_post.return_value = mock_response

        ftrack_tracking = FtrackTracking(Environment())
        ...
        ftrack_tracking.insert_version(version_number, video_path)

        mock_post.assert_called_once_with(
            f"https://your-ftrack-instance.com/api/v1/projects/{project_id}/versions",
            json={
                "data": {
                    "type": "versions",
                    "attributes": {
    ...
    def test_insert_version_failure(self, mock_post):
        """
        Test the insert_version method for the Ftrack API failure scenario.
        """
        mock_response = Mock()
